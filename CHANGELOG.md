...
* theme Avalonia 12.1's TableView, bump to Avalonia 12.1.2 ([#55](https://github.com/Alpaq92/Fluid.Avalonia/issues/55)) ([285d018](https://github.com/Alpaq92/Fluid.Avalonia/commit/285d0186e520bbb903a0295e53f7aeb1ff93afec))
  * **`TableView` control theme** (`Controls/TableView.axaml`). Avalonia 12.1 added a core, read-only `TableView` — a `ListBox` with XAML-defined, resizable columns — sitting between a `ListBox` and the `DataGrid`. It lives in `Avalonia.Controls`, so the **library** themes it.
  * The look matches our ListBox and DataGrid: the same bordered, rounded, opaque surface, a quiet header strip with secondary-colour semibold captions, 40 px rows on the ListBox's 16 px inset, and the `ListBoxItem`'s translucent base-accent selection.
  * Row brushes go through FluentTheme's own `TableViewRow*` keys. `TableView`, its header-hosting `ScrollViewer` and `TableViewColumnHeader` are re-authored.
  * Header hover is **matched to the bundled DataGrid's** column header: a neutral scrim at `0x19`, black in light and white in dark. The overlay scrollbars also moved off the header strip, where they had been painting across the last column.
  * **Opt-in hot reload** on the Desktop head, behind `-p:FluidAvaloniaHotReload=true` (Debug only). The package is commercial, so a plain build and CI never reference it. It does not replace the **Playground**, which is a runtime XAML sandbox for the app's own users and also ships in the WebAssembly demo. See [OVERVIEW.md](OVERVIEW.md#hot-reload-opt-in).
